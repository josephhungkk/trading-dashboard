...
async def test_three_fills_in_one_batch_all_emitted_once() -> None:
    ...
    state_cache = OrderStateCache(
        redis=redis, gateway_label="schwab", account_id="acct-1"
    )
...
async def test_new_fill_on_second_poll_emitted() -> None:
    ...
    redis = _FakeRedis()
    state_cache = OrderStateCache(
        redis=redis, gateway_label="schwab", account_id="acct-2"
    )
    await state_cache.hydrate()
...
async def test_state_cache_serialises_exec_id_set() -> None:
    ...
    cache = OrderStateCache(
        redis=redis, gateway_label="schwab", account_id="acct-3"
    )
...
async def test_state_cache_migrates_old_last_exec_id_format() -> None:
    ...
    redis = _FakeRedis()
    cache = OrderStateCache(
        redis=redis, gateway_label="schwab", account_id="acct-4"
    )
