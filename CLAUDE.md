...
- **BASE-tag startup round (5b.1 C1, ATTEMPTED TWICE → SKIPPED):** empirical pre-flight (`sidecar/scripts/base_round_preflight.py`, committed as evidence) cycled `ib.client.reqAccountUpdates(True/False, account)` per managed account against paper gateway 4002. Run 1 (sidecar live, secondary clientId): zero BASE entries. Run 2 (sidecar killed via `gsudo Stop-Process`, port 18002 free, `clientId=999` as master): zero BASE entries. The IBKR API does not surface BASE through this path regardless of master/secondary session role. The v0.5.2 `_resolve_account` fallback to `last_nlv_currency` (`9910e3b`) remains the canonical workaround. Re-attempt only if a future ib_async / IBKR API revision exposes a working round.
