...
- **BASE-tag startup round (5b.1 C1, ATTEMPTED → SKIPPED):** empirical pre-flight (`sidecar/scripts/base_round_preflight.py`, committed as evidence) cycled `ib.client.reqAccountUpdates(True/False, account)` per managed account against paper gateway 4002. `ib.accountValues()` contained zero BASE entries afterward — the IBKR API does not surface BASE through the documented per-account subscription path. The v0.5.2 `_resolve_account` fallback to `last_nlv_currency` (`9910e3b`) remains the canonical workaround. Re-attempt only if a future ib_async / IBKR API revision exposes a working round.
