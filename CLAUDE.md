...
- **SSE wire shape:** `GET /api/orders/events` emits `id: <event_id>\nevent: order.update\ndata: <json>\n\n` frames + 10s `: heartbeat\n\n` keepalives. `?account_id=<uuid>` scopes to `orders:events:account:<id>`; absent → fleet-wide `orders:events:fleet`. `Last-Event-ID` header replays from `order_events WHERE id > N`. Per-client queue maxsize 256; overflow → drop with `event: error\ndata: slow_client` final frame and `sse_dropped_clients_total++`.
