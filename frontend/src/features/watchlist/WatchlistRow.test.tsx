...
function makeRow(overrides: Partial<WatchlistRowData> = {}): WatchlistRowData {
  return {
    symbol: 'AAPL',
    ...overrides,
  };
...
describe('WatchlistRow', () => {
  it('renders View Chart link with correct canonical_id', async () => {
    renderWithRouter(
      <WatchlistRow row={makeRow({ symbol: 'AAPL', canonical_id: 'AAPL.US' })} />,
    );
  ...
  it('omits link when canonical_id is null', async () => {
    renderWithRouter(
      <WatchlistRow row={makeRow({ symbol: 'AAPL', canonical_id: null })} />,
    );
  ...
  it('omits link when canonical_id is undefined', async () => {
    renderWithRouter(
      <WatchlistRow row={makeRow({ symbol: 'MSFT' })} />,
    );
