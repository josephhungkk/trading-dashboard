...
``real_futu``, ``real_ibkr``) and auto-skip when env vars are missing —
local ``pytest`` runs stay green even without secrets configured.
"""
...
def pytest_addoption(parser: pytest.Parser) -> None:
    parser.addoption(
        ...
        help=(
            ...
            "trail_percent_spy | moc_spy | gtd_limit_spy)"
        ),
    )
