...
**ARCHITECT-REVIEW applied:** Pass 1 (4 CRIT + 6 HIGH + 7 MED) + Pass 2 (5 CRIT + 8 HIGH + 9 MED) + Pass 3 (6 CRIT + 9 HIGH + 11 MED). All inline.
...
- 13 Prometheus metrics (11 + 2 new from CRIT-6 and HIGH-9).
...
if self._advisor_config.mode == AdvisorMode.VETO and verdict.action == "approve":
...
- Extend `PAUSE` handler to propagate `payload.get("reason", "manual")` into `bot_runs.stop_reason` and status pubsub frame.
- **No `RELOAD_CONFIG` command** (CRIT-2d): config changes are handled by STOP+START (the supervisor's `PUT /api/bots/{id}` handler already stops and restarts a running bot after config update). This avoids adding a new undefined control-queue command. The `RELOAD_CONFIG` reference in earlier spec revisions is removed.

#### `app/api/bots.py`

- `PUT /api/bots/{id}` — extend `BotUpdateRequest` with `advisor_config: AdvisorConfig | None`. JSONB write via `model_dump(mode="json")` (drops unknown keys from older configs — idempotent backfill). On save: if bot is running, trigger STOP+START cycle (same as any config change) so the child picks up the new advisor config.  
...
Columns: timestamp, verdict badge (green=approve / red=veto / amber=fail_open), symbol, side, qty, `account_gate_outcome` badge, latency ms, provider, reasoning preview (80 chars). Click → `AdvisorDecisionDrawer`. Cursor pagination via `next_cursor` (base64url decoded by `api.ts`).
...
Generated via `gen-types.sh` where possible. Hand-authored fallback for `AdvisorVetoedResult` (not in OpenAPI). Strict TypeScript. `qty`/`limit_price`/`stop_price` typed as `string`. `ai_completion_id` replaced with `ai_completion_ts: string | null` + `ai_completion_request_id: string | null`. `account_gate_outcome` typed as `'approved' | 'warned' | 'blocked' | 'not_evaluated' | 'error'`.
...
strategy.on_bar()
  ...
  → AdvisorService.review()
      ...
      _persist on independent AsyncSession (commit; account_gate_outcome='not_evaluated')
...
**Frame version:** `useAdvisorStream` drops frames where `v !== 1`. Bumping `v` requires a new client release.
...
13 metrics total. Does not duplicate provider-level latency from Phase 11a `ai_completions`.
...
| Module | Tests |
|---|---|
| `types.py` | Pydantic: veto+empty-reasoning rejected; approve/veto/fail_open accepted; confidence bounds; advice_tags filtered; `qty` round-trips as string; `daily_budget_usd` JSONB-as-string; `AICapability` enum rejects bad string; `ContextSummary` validates; `account_gate_outcome` CHECK values (9) |
...
| `api/bots.py` | PUT updates advisor_config with CSRF nonce; rejects invalid mode enum; rejects invalid `capability` string; GET decisions cursor-paginates (base64url decode); GET detail 404 cross-bot; GET advisor-feed admin-only; `account_gate_outcome` returned in detail (7) |
| `api/ws_bots.py` advisor WS | Subscribes channel; conflates 500ms; 50-conn cap; closes on JWT expiry; frame includes `account_gate_outcome` (5) |
| Alembic 0063 | up→down→up clean; `stop_reason` CHECK includes `advisor_auto_pause`; `advisor_config_mode_check` rejects bad mode; index present; no FK on `bot_run_id`; no FK on `ai_completion` columns; `account_gate_outcome` CHECK values; `advisor_config_mode_check` uses `?` operator (8) |
| `auto_pause.py` + supervisor | `stop_reason='advisor_auto_pause'` written without IntegrityError; pause propagates `reason` to status frame; STOP+START handles config update (3) |
| Budget reconcile task | reconcile loop corrects over-estimate; corrects under-estimate; gauge updates (3) |
| **Total** | **~86** |

*(target ~63 from post-pass-1 spec + ~23 additional from passes 2+3)*

### Frontend (~24 tests, Vitest + RTL)

| Component/hook | Tests |
|---|---|
| `AdvisorConfigForm` | All fields rendered; `local_only` checkbox maps correctly; `min_veto_confidence` slider; `capability` maps to `AICapability`; submit calls mintCsrfNonce; disabled during save; validates timeout bounds; mode/capability are `<select>` elements (8) |
| `AdvisorDecisionsTable` | Verdict badges; `account_gate_outcome` badge; cursor pagination (base64url next_cursor); empty state; click opens drawer (5) |
| `AdvisorDecisionDrawer` | Escape closes; aria-modal; intent JSON in `<pre>`; advice_tags as chips; reasoning is text node (no dangerouslySetInnerHTML); `account_gate_outcome` shown (6) |
...
|---|---|---|---|
| **A — DB + types + context builder** | Alembic 0063 (incl. `stop_reason` widen + JSONB CHECK + `account_gate_outcome` column), `types.py`, `context_builder.py`, `prompts.py`, tests | Qwen | — |
| **B — Service + auto-pause + metrics + budget reconcile** | `service.py` (incl. `update_account_gate_outcome`), `auto_pause.py`, `metrics.py`, budget reconcile task, tests | Codex | after A |
| **C — BotContext + BaseStrategy + Supervisor wiring** | `base.py`, `context.py` (bot-cap → advisor → state-drift check → facade → gate outcome update), `supervisor.py` (PAUSE propagates reason; no RELOAD_CONFIG), integration tests | Opus direct | **after Phase 19.1** |
| **D — REST + WS API** | `api/bots.py`, `api/ws_bots.py`, tests | Codex | after A + B |
| **E — Frontend** | `services/advisor/` (gen-types.sh), 5 components (incl. account_gate_outcome badges, a11y form), hook (v-frame guard), page, `BotDetailPage` tab | Codex | after D |
...
| CRIT-2: Wrong auto-pause envelope + `stop_reason` CHECK + `reason` propagation | Correct XADD `{"cmd":"PAUSE","reason":"advisor_auto_pause"}` envelope; 0063 widens CHECK; supervisor propagates `reason` to both `bot_runs.stop_reason` and `bot:status:*` pubsub; RELOAD_CONFIG dropped in favour of STOP+START |
...
| MED-5: Missing concurrency tests | ~23 new tests added vs. pass-1 spec; total ~86 BE |
...
- **LLM Advisor (Phase 21a, shipped v0.21.0):** Per-bot opt-in advisor intercepts
  ...
  gate; optimistic Redis budget counter + 5-min reconcile loop. Alembic 0063:
  `bot_advisor_decisions` plain table (no FK to hypertable columns; partial index on bot_run_id;
  `account_gate_outcome` column); `bots.advisor_config` JSONB with `?'mode'` CHECK;
  ...
  (pubsub `bot:advisor:{bot_id}`, 500ms conflation, 50-conn cap; v-frame guard). REST:
  cursor list/detail (base64url cursor) + admin feed. 13 Prometheus metrics under `advisor_*`.
  ~86 BE / ~27 FE tests. **Gated on Phase 19.1** (supervisor child build-out). Deferred:
  SHADOW, async-parallel, human override, advisor-in-backtest, Telegram.
```
