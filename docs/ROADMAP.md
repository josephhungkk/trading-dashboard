...
| # | Tag | Theme | Headline deliverables |
|---|---|---|---|
| **7a** | 0.7.0 | **Streaming quote engine + IBKR/Futu sources** | Subscription registry (refcount), Redis quote bus `quote.<source>.<canonical_id>`, frontend WebSocket gateway with conflation (4–10/s), `instruments` + `symbol_aliases` schema (Alembic 0008), stale detection. Live ticks on watchlist/positions/trade-ticket. Source enum is open-set. |
| **7b** | 0.7.1 | **Schwab connect — data + read-only** | `sidecar_schwab/` on port 18006. OAuth + manual re-auth UI for the 7-day refresh-token wall + opt-in Tier-2 Playwright auto-refresher (feature-flagged). `Configure` RPC, `ListAccounts`, `StreamQuotes` (LEVELONE_*, CHART_EQUITY, ACCT_ACTIVITY). Quote-source-router flips US→Schwab. Trade execution returns UNIMPLEMENTED. **Saves IBKR data fees from v0.7.1.** |
